...
export class DriverRideAcceptanceService {
  ...
  onModuleInit() {
    this.logger.log(
      "Driver ride acceptance service initialized. Awaiting driver authentication.",
    );
    // Cache the matchmaking URL on initialization instead of fetching it on every request
    this.matchmakingUrl = this.envService.getString(
  ...
  async subscribeForDriver(_driverId: string): Promise<void> {
    this.logger.log(
      `Driver ${_driverId} is now ready to receive ride events via unified channel`,
    );
  }
  ...
  async acceptRide(
    ...
    this.logger.debug(
      `Calling matchmaking service for ride acceptance. rideUUID=${ride.rideUUId}`,
    );
    ...
    try {
      ...
      if (result?.success) {
        this.logger.log(
          `Driver ${driverId} successfully accepted ride ${rideId} via matchmaking service`,
        );
        ...
        const acceptDetails: DriverAcceptDetails = {
          ...
          estimatedTimeInMinutes:
            mmDetails?.estimatedTimeInMinutes ||
            ride.estimatedTimeInMinutes ||
            0,
        ...
        await this.rideChannelService.publishRideEvent(
          ride.rideUUId,
          "driver-response",
          { driverId, action: DriverActionEnum.ACCEPT },
        );
        ...
        return {
          success: false,
          message:
            result?.message || "Failed to accept ride via matchmaking service",
        };
      }
    } catch (error: any) {
      ...
      return {
        success: false,
        message: "Failed to communicate with matchmaking service",
      };
    }
  }
  ...
  async rejectRide(
    ...
    try {
      ...
      if (result?.success) {
        this.logger.log(
          `Driver ${driverId} successfully rejected ride ${rideId}`,
        );
        await this.rideChannelService.publishRideEvent(
          ride.rideUUId,
          "driver-response",
          { driverId, action: DriverActionEnum.REJECT },
        );
        ...
        return {
          success: false,
          message:
            result?.message || "Failed to reject ride via matchmaking service",
        };
      }
    } catch (error: any) {
   ...
   */
  async buildAcceptDetails(
    ride: RidesDocument,
    driverId: string,
  ): Promise<DriverAcceptDetails> {
    const driverObjectId = new Types.ObjectId(driverId);

    // Fetch all required data in parallel for better performance
    const [driverUser, driverDetails, vehicle, passengerUser] =
      await Promise.all([
    ...
    try {
      ...
      if (!result) {
        const errorMsg =
          ...
          "Failed to complete ride via matchmaking service";
        this.logger.error(
          `Matchmaking completeRide failed: ${JSON.stringify(response.data?.errors)}`,
        );
   ...
   */
  private async callMatchmakingService<T = any>(variables: {
    rideUUID: string;
    driverId: string;
    action: string;
    rideId?: string;
    ...
    try {
      ...
      this.logger.error(
        `Matchmaking service call failed: ${error?.message || error}`,
      );
      throw error;
    }
  }
